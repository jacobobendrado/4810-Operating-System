...
```shell
docker build -t osdev-build-environment .
```

4. Run the docker image ensuring that the `-v` flag contains the path to the ShompOS source code.
```shell
sudo docker run -it --rm \
    -v ./4810-Operating-System/ShompOS:/src \
    -w /src \
    osdev-build-environment
```
